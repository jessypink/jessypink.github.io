...
function updateWeekdayTextSmooth(newText) {
    const wrapper = document.querySelector('.weekday-wrapper');
    ...
    // Устанавливаем новый текст сразу, чтобы измерить его ширину
    weekdayDisplay.textContent = newText;

    // Измеряем новую ширину
    wrapper.style.width = 'auto';
    const newWidth = wrapper.offsetWidth;

    // Возвращаем к текущей ширине, чтобы запустить анимацию
    ...
    // После окончания анимации (300 мс) сбрасываем ширину в auto,
    // чтобы контейнер адаптировался при следующем изменении
    setTimeout(() => {
        wrapper.style.width = 'auto';
    }, 300);
}
...
function updateWeekdayDisplay(dateStr) {
    if (!dateStr) {
        updateWeekdayTextSmooth('');
        return;
    }
    const date = new Date(dateStr);
    const weekday = date.toLocaleDateString('ru-RU', { weekday: 'long' });
    updateWeekdayTextSmooth(weekday.charAt(0).toUpperCase() + weekday.slice(1));
}
...
document.getElementById('dateInput').addEventListener('change', (e) => {
    ...
    if (date) {
        updateSchedule(date);
        updateWeekdayDisplay(date);  // <-- сюда добавил обновление дня недели
    }
});
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    badge.innerHTML = `${day}<br>${weekday}`;
    ...
    updateWeekdayDisplay(todayStr); // <-- здесь тоже обновляем сразу
    ...
    function changeDate(days) {
        ...
        updateWeekdayDisplay(currentDate); // <-- здесь тоже обновляем сразу
    }
